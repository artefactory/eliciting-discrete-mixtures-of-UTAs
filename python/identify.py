...
class UTAsIdentification:
    ...
    def __init__(self, n_criteria, n_pieces):
        ...
        self.slopes = {
            "alpha": [[] for _ in range(n_criteria)],
            "beta": [[] for _ in range(n_criteria)],
        }
        self.slopes["alpha"][0].append(1.0)
        self.slopes["beta"][0].append(1.0)

    def identify_21(self, hdms):
        query = SingleRectangleQuery(
            criterion_i=0, criterion_j=1, min_i=0, max_i=1, min_j=0, max_j=1
        )
    ...
    def identify_22(self, hdms):
        query_1 = SingleRectangleQuery(
            criterion_i=0, criterion_j=1, min_i=0, max_i=1, min_j=1, max_j=2
        )
        crits_1, I1_1, I2_1 = query_1.query_hidden_dms(hdms)

        query_2 = NeighboringRectanglesQuery(
            bridged_criterion=1,
        ...
        case_1 = np.min(
            [
                np.abs(possible_slopes_1[0] - unidentified_slope_1)
                + np.abs(possible_slopes_1[1] - unidentified_slope_2),
                np.abs(possible_slopes_1[1] - unidentified_slope_1)
                + np.abs(possible_slopes_1[0] - unidentified_slope_2),
            ]
        )

        case_2 = np.min(
            [
                np.abs(possible_slopes_2[0] - unidentified_slope_1)
                + np.abs(possible_slopes_2[1] - unidentified_slope_2),
                np.abs(possible_slopes_2[1] - unidentified_slope_1)
                + np.abs(possible_slopes_2[0] - unidentified_slope_2),
            ]
        )
    ...
    def generic_identification(self, hdms, criterion, square):
        ...
        if opposite_criterion == 0:
            slopes_values = [
                (I1[1][1] - I1[0][1]) / (I1[0][0] - I1[1][0]),
                ((I2[1][1] - I2[0][1]) / (I2[0][0] - I2[1][0])),
            ]
        else:
            slopes_values = [
                [
                    ((I1[1][1] - I1[0][1]) / (I1[0][0] - I1[1][0]))
                    * self.slopes["alpha"][opposite_criterion][0],
                    ((I2[1][1] - I2[0][1]) / (I2[0][0] - I2[1][0]))
                    * self.slopes["beta"][opposite_criterion][0],
                ],
                [
                    ((I2[1][1] - I2[0][1]) / (I2[0][0] - I2[1][0]))
                    * self.slopes["alpha"][opposite_criterion][0],
        ...
        possible_slopes = [
            ((I1[1][1] - I1[0][1]) / (I1[0][0] - I1[1][0]))
            * self.slopes["alpha"][opposite_criterion][1],
            ((I2[1][1] - I2[0][1]) / (I2[0][0] - I2[1][0]))
            * self.slopes["beta"][opposite_criterion][1],
        ]
        other_possible_slopes = [
            ((I1[1][1] - I1[0][1]) / (I1[0][0] - I1[1][0]))
            * self.slopes["beta"][opposite_criterion][1],
            ((I2[1][1] - I2[0][1]) / (I2[0][0] - I2[1][0]))
            * self.slopes["alpha"][opposite_criterion][1],
        ...
        if opposite_criterion == 0:
            case_1 = np.min(
                [
                    np.abs(possible_slopes[0] - slopes_values[0])
                    + np.abs(possible_slopes[1] - slopes_values[1]),
                    np.abs(possible_slopes[1] - slopes_values[0])
                    + np.abs(possible_slopes[0] - slopes_values[1]),
                ]
            )

            case_2 = np.min(
                [
                    np.abs(other_possible_slopes[1] - slopes_values[0])
                    + np.abs(other_possible_slopes[0] - slopes_values[1]),
                    np.abs(other_possible_slopes[0] - slopes_values[0])
                    + np.abs(other_possible_slopes[1] - slopes_values[1]),
                ]
            )
        else:
            case_1 = np.min(
                [
                    np.abs(possible_slopes[0] - slopes_values[0][0])
                    + np.abs(possible_slopes[1] - slopes_values[0][1]),
                    np.abs(possible_slopes[0] - slopes_values[1][0])
                    + np.abs(possible_slopes[1] - slopes_values[1][1]),
                ]
            )

            case_2 = np.min(
                [
                    np.abs(other_possible_slopes[1] - slopes_values[0][0])
                    + np.abs(other_possible_slopes[0] - slopes_values[0][1]),
                    np.abs(other_possible_slopes[1] - slopes_values[1][0])
                    + np.abs(other_possible_slopes[0] - slopes_values[1][1]),
                ]
            )
        if case_1 < case_2:
